...
for k in Vminus1 # sources
	@constraint(model, sum(f[(j,1),k] for j in V if   (j,1) in A) 
	- sum(f[(1,j),k] for j in V if  (1,j) in A)==-1)
end


for k in Vminus1, i in Vminus1 # balances
  if i!=k 
		@constraint(model, sum(f[(j,i),k] for  j in V if (j,i) in A) 
		- sum(f[(i,j),k] for  j in V if  (i,j) in A)==0)
	end
end


for k in Vminus1 # sinks
  @constraint(model, sum(f[(j,k),k] for j in V if (j,k) in A) 
	- sum(f[(k,j),k] for j in V if (k,j) in A)==1)
end
