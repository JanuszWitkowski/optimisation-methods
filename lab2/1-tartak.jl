...
function unused(x, cuts, demand, n_of_cuts, n_of_widths)
    return sum(sum(cuts[i, j] * x[i] for i in 1:n_of_cuts) - demand[j] for j in 1:n_of_widths)
end
...
# @objective(model, Min, sum(sum(cuts[i, j] * x[i] for i in 1:n_of_cuts) - demand[j] for j in 1:n_of_widths) + sum(x[i] * (main_width - sum(widths[j] * cuts[i, j] for j in 1:n_of_widths)) for i in 1:n_of_cuts))
@objective(model, Min, unused(x, cuts, demand, n_of_cuts, n_of_widths) + leftovers(x, main_width, widths, cuts, n_of_cuts, n_of_widths))
...
x = JuMP.value.(x)
println("Niewykorzystane: ", unused(x, cuts, demand, n_of_cuts, n_of_widths))
